...
- **flashforge** — `install.sh` asks for the FlashForge printer IP (default `192.168.1.113`; `PRINTER_IP=<ip>` for non-interactive use), saves it to `.env`/the `flashforge.printer_ip` container label, clones the upstream source and builds. It also asks for an optional camera stream URL (`CAMERA_URL=<url>`; MJPEG or plain image, e.g. a motionEye stream port) — the GUI's video section then shows that stream instead of the printer's own camera at `http://<printer-ip>:8080/?action=stream`, which only the 5M Pro/camera accessory serves; `-` clears a saved URL. GUI at `http://<host>:9876/en/<printer-ip>`, raw API at `http://<host>:9876/api/execute/<printer-ip>/progress` (also `temperature`, `position`, `info`, `status`). Re-run `install.sh` to change the printer IP or camera URL or pull upstream updates, then re-run `homarr/findtargetcontainers.sh` to refresh the tile.
