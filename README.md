...
| `wallabag/` | `wallabag/wallabag` | Read-it-later / bookmark archiver — saves readable copies of web articles | 8085 | `.env` (auto-generated by `install.sh`) |
...
| `wallabag/` | `WALLABAG_SECRET`, `WALLABAG_DOMAIN_NAME` | Both filled in by `install.sh` — secret is generated, domain defaults to `http://<host-ip>:8085` |
...
- **calibre** — `install.sh` bootstraps everything: creates the library, skips the welcome wizard, and enables the content server (port 8081) with auto-start and **no authentication**; `local_write` lets clients also upload and edit books without login (`trusted_ips` alone is not enough — it only bypasses auth when auth is enabled). Anyone on the LAN has full read/write access, so do not port-forward 8081. The full desktop runs in the browser at **https://\<host\>:8092** (no VNC client needed) — HTTPS is mandatory, the streaming stack rejects plain http ("This application requires a secure connection"); accept the self-signed cert warning once. Port 8091 (http) only works behind a TLS reverse proxy. Upload/download files via the KasmVNC side panel; convert books via the desktop UI (Convert books button). The content server (8081) works over plain http. arm64 image — works on Pi 4/5 running 64-bit OS.
- **homarr** — `install.sh` prompts for admin credentials and creates the user directly in the DB (`HOMARR_ADMIN_USER`/`HOMARR_ADMIN_PASSWORD` for non-interactive use); the web onboarding wizard is skipped entirely. Then `bash homarr/findtargetcontainers.sh` scans the local Docker node and adds every container as an app tile on the default board (URL from published ports, icon from dashboard-icons CDN). Re-runs add new containers and remove gone ones; onboarding-wizard leftovers (sample links, docker-import tiles, default widgets) are pruned (`--keep-defaults` to keep them), manually created tiles are never touched. The target board is made public and set as server-wide home board so phones/TVs can view it without login (`--keep-private` to opt out; editing still needs an account). `--dry-run` previews, docker labels `homarr.ignore` / `homarr.url` / `homarr.icon` override per container. Built-in special cases: background containers (twingate, cloudflared, portainer agent) are skipped; calibre gets its https UI (`:8092`) plus a content-server tile (`:8081`); netwatch gets `http://<host>:8095/` (host network mode, no published ports); marcujump gets an extra Handbuch tile; FlashForgeAdventurer5MAPI gets `http://<host>:9876/en/<printer-ip>` (printer IP from the `flashforge.printer_ip` label). After each sync the board is compacted (gaps from removed tiles closed, tiles repacked top-left). Details in `homarr/README.md`.
...
- **wallabag** — `install.sh` generates `WALLABAG_SECRET`, sets `WALLABAG_DOMAIN_NAME` to `http://<host-ip>:8085` (override with `WALLABAG_DOMAIN_NAME=<url>`; it must match the URL browsers and apps use, otherwise share links and the API break), waits for the first-run database setup, and then changes the password of the built-in `wallabag` account (interactive prompt, or `WALLABAG_ADMIN_PASSWORD=<pw>` for non-interactive use). Login user stays `wallabag`. Uses the SQLite backend, so no separate database container is needed; data lives in the `wallabag_data` volume. Public sign-up is disabled — add further users under Settings → Users. First start takes a few minutes on a Pi while the schema is created. Save articles via the browser extensions or mobile apps by pointing them at the same URL.
