...
- **homarr** — `install.sh` prompts for admin credentials and creates the user directly in the DB (`HOMARR_ADMIN_USER`/`HOMARR_ADMIN_PASSWORD` for non-interactive use); the web onboarding wizard is skipped entirely. Then `bash homarr/findtargetcontainers.sh` scans the local Docker node and adds every container as an app tile on the default board (URL from published ports, icon from dashboard-icons CDN). Re-runs add new containers and remove gone ones; onboarding-wizard leftovers (sample links, docker-import tiles, default widgets) are pruned (`--keep-defaults` to keep them), manually created tiles are never touched. `--dry-run` previews, docker labels `homarr.ignore` / `homarr.url` / `homarr.icon` override per container. Built-in special cases: background containers (twingate, cloudflared, portainer agent) are skipped; calibre gets its https UI (`:8092`) plus a content-server tile (`:8081`); marcujump gets an extra Handbuch tile. Details in `homarr/README.md`.
