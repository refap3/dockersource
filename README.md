...
| `calibre/` | `lscr.io/linuxserver/calibre` | Full Calibre desktop streamed to the browser (KasmVNC) + content server for browsing/uploading/downloading books | 8091 (UI), 8092 (UI https), 8081 (content server) | `.env` (auto-generated by `install.sh`) |
...
- **calibre** — `install.sh` bootstraps everything: creates the library, skips the welcome wizard, enables the content server (port 8081) with auto-start, and creates an upload-capable user (credentials in `calibre/.env`). The full desktop runs in the browser at port 8091 (no VNC client needed); upload/download files via the KasmVNC side panel. Convert books via the desktop UI (Convert books button). arm64 image — works on Pi 4/5 running 64-bit OS.
