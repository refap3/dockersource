...
- **homarr** — after install, `bash homarr/findtargetcontainers.sh` scans the local Docker node and adds every container as an app tile on the default board (URL from published ports, icon from dashboard-icons CDN). Re-runs add new containers and remove gone ones; manually created tiles are never touched. `--dry-run` previews, docker labels `homarr.ignore` / `homarr.url` / `homarr.icon` override per container. Details in `homarr/README.md`.
